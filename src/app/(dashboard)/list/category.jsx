...
export default function MenuInfo() {
  ...
  const timer = useRef();
  const adminRestaurantInfo = useSelector(
    (state) => state.adminRestaurantInfo.adminRestaurantInfo
  );
  ...
  useEffect(() => {
    ...
    if (location.state?.data.id) {
      ...
      for (let i = 0; i < location.state?.data.menuItems.length; i++) {
      ...
      setUploadedData(location.state?.data);
      setFormMenuErrors(errorObj);
      setFormMenuData(location.state?.data);
      setCategoryName(location.state?.data?.name);
      setSelectedCategoryLogo(location.state?.data?.categoryLogo);
    }
  }, [location]);
